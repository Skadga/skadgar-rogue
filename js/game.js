// ========== GAME.JS ==========
// ИСПРАВЛЕННАЯ ВЕРСИЯ - РАБОТАЕТ ИНВЕНТАРЬ

// Объявляем переменные
let isPaused = false;
window.isPaused = false; // Синхронизация
...
let moveTargetTile = null;

// Функция для обновления глобальных переменных после изменения размера окна
function updateGlobalWindowSize() {
    if (typeof updateOffsets === 'function') updateOffsets();
    if (typeof clearCache === 'function') clearCache();
    if (typeof updateCamera === 'function') updateCamera();
}

// Инициализация плавающих окон
if (typeof initFloatingWindows === 'function') {
    initFloatingWindows();
}
...
function resetGame() {
    ...
    if (window.autoAttackEnabled) {
        if (typeof toggleAutoAttack === 'function') toggleAutoAttack();
    }
}
...
window.addEventListener('keydown', (e) => { 
    // Клавиша I - ИНВЕНТАРЬ (открыть/закрыть)
    if (e.code === 'KeyI') {
        e.preventDefault();
        e.stopPropagation();
    ...
    // Клавиша ESC - закрыть инвентарь/магазин или пауза
    if (e.code === 'Escape') { 
        e.preventDefault();
    ...
    if (e.code === 'KeyG' && !isPaused && window.gameActive) {
        e.preventDefault();
        if (typeof shopKeeper !== 'undefined' && shopKeeper.isNearby) {
            if (typeof openShop === 'function') openShop();
        } else if (typeof addPickupEffect === 'function') {
            addPickupEffect(window.player.x, window.player.y, "Подойди к торговцу (G)");
        }
        return;
    }
});
...
function gameLoop(timestamp) {
    ...
    if (typeof drawAllWithDepth === 'function') {
        drawAllWithDepth();
    } else {
...
// Обновление UI окон
setInterval(() => {
    if (typeof updateFloatingWindowsUI === 'function') {
        updateFloatingWindowsUI();
    }
}, 100);

// ========== ФУНКЦИЯ КНОПКИ АВТО-АТАКИ ==========
function initAutoAttackButtons() {
    const autoBtn = document.getElementById('autoAttackBtn');
    
    if (!autoBtn) {
        setTimeout(initAutoAttackButtons, 500);
        return;
    }
    
    console.log('initAutoAttackButtons - found button');
    ...
    newAutoBtn.onclick = function(e) {
        ...
        if (typeof window.toggleAutoAttack === 'function') {
            window.toggleAutoAttack();
        }
    };
    
    if (typeof window.updateAutoAttackButtonUI === 'function') {
        window.updateAutoAttackButtonUI();
    }
}

window.initAutoAttackButtons = initAutoAttackButtons;
